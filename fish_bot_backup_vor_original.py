...
def _klick_loop(maus, mess_zustand, fertig_event):
    ...
    while not fertig_event.is_set():
        ...
        if (popup is not None and fisch_glatt is not None and steht_still
            ...
            if im_ring and sperre_abgelaufen:
                ...
                _logger.info("Fisch im Kreis + Stillstand bei (%d, %d) [geglaettet]",
                             bildschirm_x, bildschirm_y)
                _logger.info("maus_zieht_zu=(%d, %d)", bildschirm_x, bildschirm_y)
                maus_bewegen(bildschirm_x, bildschirm_y, maus)
                ...
                _logger.info("KLICK GESENDET: ziel=(%d, %d) erfolg=%s",
                             bildschirm_x, bildschirm_y, klick_erfolg)
                if not klick_erfolg:
                ...
                _logger.info("Klick %d in diesem Popup", klicks_in_diesem_popup)
...
def zustand_fischen(maus):
    ...
    mess_thread = threading.Thread(
        ...
        daemon=True, name="FischMessLoop")
    mess_thread.start()

    _klick_loop(maus, mess_zustand, fertig_event)
...
def zustand_warte_einholen(maus):
    ...
    Start")."""
    warte_mit_bonus(WARTE_EINHOLEN_BASIS)
    if _gestoppt():
        return ZUSTAND_GESTOPPT
    leertaste_tippen(maus)
    return ZUSTAND_PRUEFE_POPUP


def zustand_pruefe_popup(maus):
    ...
    warte_mit_bonus(PRUEFE_POPUP_BASIS)
    if _gestoppt():
        return ZUSTAND_GESTOPPT
...
def bot_schleife(maus):
    ...
    while zustand != ZUSTAND_GESTOPPT:
        _logger.info("Zustand: %s", zustand)
        zustand = _ZUSTAND_HANDLER[zustand](maus)

    _logger.info("Bot sauber gestoppt (bot_anhalten() aufgerufen)")
    return "GESTOPPT"


def bot_starten(port=None, maus=None):
    ...
    Returns:
        str: "VERBINDUNGSFEHLER" | "GESTOPPT"
    """
    _stop_event.clear()
    ...
    _csv_logger_init()
    _logger.info("FishBot laeuft.")
    try:
        ergebnis = bot_schleife(maus)
    finally:
        if eigene_verbindung:
            maus.schliessen()
        if _csv_datei is not None:
            _csv_datei.close()
    return ergebnis
